...
function Mount-WSLDev {
    <#
    .SYNOPSIS
        Finds the Forge SSD by serial number, attaches it to WSL, and mounts at /mnt/forge.
    .DESCRIPTION
    ...
    $logFile = Join-Path $logDir 'mount-forge.log'
    ...
    if (-not (Test-IsAdmin)) {
        ...
        Write-DevLog 'Elevating to mount Forge drive...' -Level WARN -Source 'Mount' -LogFile $logFile
        ...
        $status = wsl.exe -d Ubuntu -e bash -c "mountpoint -q /mnt/forge && echo 'YOURMOUNT_YES' || echo 'YOURMOUNT_NO'" 2>&1
        if ($status -match 'YOURMOUNT_YES') {
            Write-DevLog 'Forge drive: MOUNTED' -Source 'Mount' -LogFile $logFile
        } else {
            Write-DevLog 'Forge drive: NOT MOUNTED' -Level ERROR -Source 'Mount' -LogFile $logFile
        }
        return
    }
    ...
    $mountCheck = (wsl.exe -d Ubuntu -e bash -c "mountpoint -q /mnt/forge && echo YOURMOUNT_YES || echo YOURMOUNT_NO" 2>&1) -join ' '
    if ($mountCheck -match 'YOURMOUNT_YES') {
        Write-DevLog 'Forge drive already mounted at /mnt/forge' -Source 'Mount' -LogFile $logFile
        return
    }
    ...
    # --- Find the Forge drive by serial number ---
    $devDisk = Get-Disk | Where-Object { $_.SerialNumber -match $DevDriveSerial }

    if (-not $devDisk) {
        Write-DevLog "Forge drive not found (serial: $DevDriveSerial). Is it plugged in?" -Level WARN -Source 'Mount' -LogFile $logFile
        return  # Not an error — drive simply not connected
    }

    $driveNumber = $devDisk.Number
    Write-DevLog "Found Forge drive at PHYSICALDRIVE$driveNumber (Status: $($devDisk.OperationalStatus))" -Source 'Mount' -LogFile $logFile
    ...
    if (-not $attached) {
        Write-DevLog "Failed to attach Forge drive after $maxRetries attempts" -Level ERROR -Source 'Mount' -LogFile $logFile
        $err = [System.Management.Automation.ErrorRecord]::new(
    ...
    for ($i = 1; $i -le 10; $i++) {
        ...
        $blkCheck = wsl.exe -d Ubuntu -e bash -c "blkid -L Forge 2>/dev/null && echo FOUND || echo NOTFOUND" 2>&1
    ...
    Write-DevLog 'Mounting partition inside WSL at /mnt/forge...' -Source 'Mount' -LogFile $logFile
    $wslResult = wsl.exe -d Ubuntu -e sudo /usr/local/bin/mount-forge-drive.sh 2>&1
    ...
    # --- Verify ---
    $verify = (wsl.exe -d Ubuntu -e bash -c "mountpoint -q /mnt/forge && echo SUCCESS || echo FAILED" 2>&1) -join ' '
    if ($verify -match 'SUCCESS') {
        Write-DevLog 'WSL Forge drive mounted successfully at /mnt/forge' -Source 'Mount' -LogFile $logFile
    } else {
        Write-DevLog 'Mount verification failed' -Level ERROR -Source 'Mount' -LogFile $logFile
        $err = [System.Management.Automation.ErrorRecord]::new(
            [System.Exception]::new('Mount verification failed — /mnt/forge not mounted'),
