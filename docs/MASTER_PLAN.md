...
- **Bootstrap scripts** — `bootstrap.sh` (the cross-platform forward path: dispatches on `REVKIT_OS` from `lib/platform.sh`, supports macOS + native Linux + WSL2) and the legacy `bootstrap-wsl.sh` (now a deprecation shim that execs `bootstrap.sh`, kept for the deployed `~/.revealui/bootstrap-wsl.sh` invocation path). Installs Nix + direnv, fnm, Node 24, pnpm, base shell aliases. WSL-only steps (sudoers, boot optimization, sandbox init) gated behind `revkit_is_wsl`.
...
- **Neutral configs + per-user includes** — tracked configs under `shell/config/` carry no personal identity; per-machine values live machine-local in `~/.config/revkit/` (`identity.gitconfig`, `ssh.local`), wired via git `include.path` + ssh `Include`. (Replaced the TOML-profile + `scripts/render.sh` subsystem — `profiles/`, `templates/`, `scripts/render.sh` were removed in #64/#65.)
- **Tier detection** in `shell/shellrc.d/00-base.sh` — `DEVKIT_TIER=T0` (no sandbox drive) or `T1` (mounted).
...
| `bootstrap-wsl.sh` legacy invocation path | Built | High — now a shim that execs `bootstrap.sh` (#72) |
...
| Neutral configs + per-user `include.path` | Built | High — bootstrap git/ssh include step |
...
- **Reproducibility**: every dev workstation in RevFleet builds from the same neutral RevKit configs (Nix version, Node version, pnpm version, shell aliases, PATH order); per-user values stay machine-local
...
- Repo-truth reconciliation landed on `test`: **#72** (`bootstrap.sh` M-11 CI coverage + `bootstrap-wsl.sh` → shim), **#73** (this plan), **#74** (MASTER_SPEC + README to cross-platform reality). The docs now match shipped reality.
...
*(was Phase 0)* Bootstrap for Joshua's primary + secondary machines; deployed `~/.revealui/`. Tier detection, boot optimization, weekly backup. (Originally TOML-profile-driven; that mechanism was removed in #64/#65 — see Current Reality.)
...
- Onboarding docs for the neutral-config + per-user-`include.path` model (orgs other than RevealUI Studio)
