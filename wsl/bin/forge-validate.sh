#!/bin/bash
# Forge environment validation — health checks for DevKit tiers
# Installed to /usr/local/bin/ by bootstrap-wsl.sh
# Usage: forge validate [--verbose]
...
for arg in "$@"; do
    case "$arg" in
        --verbose|-v) VERBOSE=1 ;;
        *) echo "Usage: forge validate [--verbose]" >&2; exit 1 ;;
    esac
done
...
if [ -n "${FORGE_DATABASE_URL:-}" ] && [ -n "${FORGE_REDIS_URL:-}" ]; then
    check_pass "Connection string env vars set"
else
    check_fail "Connection strings missing (DATABASE_URL=${FORGE_DATABASE_URL:+set}, REDIS_URL=${FORGE_REDIS_URL:+set})"
fi

# --- Tier-specific checks ---

if [ "${DEVKIT_TIER:-}" = "T0" ]; then
    section "T0 checks (Forge drive not mounted)"

    # 10. REVEALUI_FORGE_MOUNTED unset
    if [ -z "${REVEALUI_FORGE_MOUNTED:-}" ]; then
        check_pass "REVEALUI_FORGE_MOUNTED unset"
    else
        check_fail "REVEALUI_FORGE_MOUNTED should be unset at T0 (value=${REVEALUI_FORGE_MOUNTED})"
    fi

    # 11. forge up refuses with tier error
    forge_err="$(forge-services.sh up 2>&1 || true)"
    if echo "$forge_err" | grep -qi "T0\|tier\|not mounted"; then
        check_pass "forge up correctly refuses at T0"
    else
        check_fail "forge up did not refuse at T0"
    fi

    # 12. /mnt/forge not a mountpoint
    if ! mountpoint -q /mnt/forge 2>/dev/null; then
        check_pass "/mnt/forge not mounted"
    else
        check_fail "/mnt/forge is mounted but tier is T0"
    fi

elif [ "${DEVKIT_TIER:-}" = "T1" ]; then
    section "T1 checks (Forge drive mounted)"

    # 13. /mnt/forge is a mountpoint
    if mountpoint -q /mnt/forge 2>/dev/null; then
        check_pass "/mnt/forge is mounted"
    else
        check_fail "/mnt/forge not mounted but tier is T1"
    fi

    # 14. REVEALUI_FORGE_MOUNTED=1
    if [ "${REVEALUI_FORGE_MOUNTED:-}" = "1" ]; then
        check_pass "REVEALUI_FORGE_MOUNTED=1"
    else
        check_fail "REVEALUI_FORGE_MOUNTED not set to 1 (${REVEALUI_FORGE_MOUNTED:-<unset>})"
    fi

    # 15. Forge directory structure exists
    dirs_ok=true
    for d in databases/postgres databases/redis models cache; do
        if [ ! -d "/mnt/forge/$d" ]; then
            check_fail "Missing directory: /mnt/forge/$d"
            dirs_ok=false
        fi
    done
    if $dirs_ok; then
        check_pass "Forge directory structure intact"
    fi

    # 16. Forge drive free space > 5%
    used_pct="$(df --output=pcent /mnt/forge 2>/dev/null | tail -1 | tr -d ' %')"
    if [ -n "$used_pct" ] && [ "$used_pct" -lt 95 ]; then
        free_pct=$((100 - used_pct))
        check_pass "Forge drive free space: ${free_pct}%"
    else
        check_fail "Forge drive low on space (${used_pct:-?}% used)"
    fi
    ...
    if $docker_ok; then
        ...
        if docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^forge-postgres$'; then
            if docker exec forge-postgres pg_isready -q 2>/dev/null; then
        ...
        if docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^forge-redis$'; then
            if docker exec forge-redis redis-cli ping 2>/dev/null | grep -q PONG; then
        ...
        if docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^forge-postgres$'; then
            pg_port="$(docker port forge-postgres 5432 2>/dev/null || true)"
        ...
        if docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^forge-redis$'; then
            redis_port="$(docker port forge-redis 6379 2>/dev/null || true)"
