...
# macOS `ip` does not exist. This fragment also sets a WSL-specific
# XDG_RUNTIME_DIR. So no-op entirely off WSL, keyed on the same /run/WSL
# marker lib/platform.sh uses.
