...
_FLEET_PIN="$(rfg_infer_fleet_from_path "$SCRIPT_DIR" 2>/dev/null || true)"
...
if [ -f "$SCRIPT_DIR/shell/lib/grok-attach.sh" ]; then
  ...
  if [ -n "${_FLEET_PIN:-}" ]; then
    ...
    _product="$(rfg_resolve_fleet_root 2>/dev/null || true)"
...
if [ -z "${REVEALFLEET_ROOT:-}" ] && type rfg_resolve_fleet_root >/dev/null 2>&1; then
  REVEALFLEET_ROOT="$(rfg_resolve_fleet_root || true)"
fi
